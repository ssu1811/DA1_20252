...
TRAINING_PIPELINE_VERSION = "return-target-lstm-v3"
...
def apply_chart_interactions(fig: go.Figure) -> go.Figure:
    fig.update_layout(
        ...
        legend=dict(font=dict(color="#17202a")),
    )
    fig.update_xaxes(
        color="#17202a",
        title_font=dict(color="#17202a"),
        tickfont=dict(color="#17202a"),
        gridcolor="#d9e0ea",
...
def prediction_chart(predictions: pd.DataFrame, title: str) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(size=4, color="#111827", opacity=0.7),
            hovertemplate=(
                "Ngày: %{x|%Y-%m-%d}<br>"
                "Giá thực tế: %{y:,.2f}"
                "<extra></extra>"
...
def show_features(feature_df: pd.DataFrame, symbol_features: pd.DataFrame, symbol: str) -> None:
    ...
    st.subheader(f"Bảng các đặc trưng gần nhất - {symbol}")
    display_columns = ["date", "symbol", *FEATURE_COLUMNS, TARGET_COLUMN, "target_return", "target_up"]
